...
from typing import Tuple, Dict, Iterator, Callable, Any
...
from sodium.torch_ex.extra.detection.d2c import d2c_workloads_maps
from sodium.torch_ex.loss.detection import get_loss_from_dict, LossManager
from sodium.torch_ex.models.schemas import LitAutoState, LitAutoHelperSchema, LitAutoModelSchema
from sodium.torch_ex.optim import OptimCH
from sodium.torch_ex.score.detection import Score, get_score_mean_ap_iou_ciou, ScoreManager, \
    get_predictions_iou_threshold
from sodium.torch_ex.types import (PredictionTypes, BatchType, BatchIndex, ImageTypes, TargetTypes,
                                   X_True, Y_Prediction)
from sodium.types import DatasetStepLiterals, PathType
...
class LitAutoModel(BaseClass, LitAutoModelSchema):
    ...
    # porting classification requirement, from d2c algorithm.
    x_true: X_True
    ...
    def __init__(self,
        ...
        if dataloaders is None:
            dataloaders = dict(train=None, training=None, valid=None, validation=None, test=None, testing=None)
        ...
        # self.ams_grad = False
        self.iou_threshold = 0.5  # /( 0_0)/ casino yoo!.
    ...
    def predictions_fix(self, predictions: PredictionTypes, device: torch.device | None = None) -> PredictionTypes:
        ...
        predictions = self.targets_fix(predictions, device)

        # using like nms.
        predictions = get_predictions_iou_threshold(predictions, iou_threshold=iou_threshold)
        return predictions
    ...
    def training(self, batch: BatchType, batch_idx: BatchIndex) -> Tensor:  # handle for "training"
        ...
        with torch.enable_grad():
            ...
            loss = get_loss_from_dict(loss_dict)
    ...
    def evaluation(self, batch: BatchType, batch_idx: BatchIndex = -1) -> PredictionTypes:
        """
    ...
    def validation(self, batch: BatchType, batch_idx: BatchIndex) -> Tuple[Score, Tuple[X_True, Y_Prediction]]:
        ...
        # Detection Format Into Classification Format. (make it possible)
        labels = d2c_workloads_maps(targets, predictions)

        # Return.
        return score, labels

    def testing(self, batch: BatchType, batch_idx: BatchIndex) -> Tuple[Score, Tuple[X_True, Y_Prediction]]:
        """
    ...
    def testing_task(self) -> Tuple[Score, Score]:
        """
            Validation Task.
        :return:
        """
    ...
    def action(self, cb: Callable[[LitAutoState], Any] | None = None):
        ...
        for _ in range(self.epochs):
            ...
            self.logger.info(f"mAP Score {score.mean_ap:.2f} Avg {avg_score.mean_ap:.2f}")
            self.logger.info(f"IoU Score {score.iou:.2f} Avg {avg_score.iou:.2f}")
            self.logger.info(f"cIoU Score {score.ciou:.2f} Avg {avg_score.ciou:.2f}")
...
class LitAutoHelper(BaseClass, LitAutoHelperSchema):
    ...
    def __init__(self,
                 ...
                 workdir: str = "torch/models/detection",
    ...
    def _cb(self, state: LitAutoState) -> None:
        ...
        for index in range(start, end):
            ...
            if check1 or check2:
                ...
                self.logger.info(f"mAP Score {score.mean_ap:.2f}")
                self.logger.info(f"IoU Score {score.iou:.2f}")
                self.logger.info(f"cIoU Score {score.ciou:.2f}")
